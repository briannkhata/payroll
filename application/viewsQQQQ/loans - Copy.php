							...
							<div class="tab-pane" id="tab_lon">	
												<div class="portlet-body">
													<div class="table-toolbar">
															...
															<div class="col-md-6">
																		<div class="btn-group">
																			<button id="sample_editable_1_new" class="btn green" onclick="add_arrear();">
														...
														$count=1;
														$amount=0;
														$ar   =   $this->db->select('*')->from('arrears')->get()->result_array();
                									    foreach($ar as $row):?>
															<tr class="odd gradeX">
																<td><?=$count++;?></td>
																...
																<td align="center">					
																<a href="javascript:;" class="btn btn-success btn-xs black" onclick="edit_arrear(<?=$row['arrear_id'];?>)" title="CLICK TO EDIT"><i class="fa fa-edit"></i> EDIT</a>				
															   	<!--a href="javascript:;" class="btn btn-success btn-xs black" onclick="view_loan(<?=$row['arrear_id'];?>)" title="CLICK FOR DETAILS"><i class="fa fa-eye"></i> VIEW</a-->				
																<a href="javascript:;" class="btn btn-danger btn-xs black" onclick="delete_arrear(<?=$row['arrear_id'];?>)" title="CLICK TO DELETE"><i class="fa fa-trash-o"></i> DELETE</a>				
															</td>
															</tr>
															<?php endforeach;?>
															</tbody>
...
<script type="text/javascript">
   var save_method; //for save method string
    ...
    {
      save_method = 'add';
      $('#formarrear')[0].reset(); // reset form on modals
      $('#arrearz').modal('show'); // show bootstrap modal
    }

		
    function edit_arrear(id)
    {
      save_method = 'update';
      $('#formarrear')[0].reset(); // reset form on modals
      ...
      $.ajax({
        url : "<?php echo site_url('staff/arrear_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
			
			$('[name="arrear_id"]').val(data.arrear_id);
            ...
            $('#arrearz').modal('show'); // show bootstrap modal when complete loaded
            $('.modal-title').text('EDIT ARREAR'); // Set title to Bootstrap modal title
     ...
     function save()
    {
      ...
      if(save_method == 'add')
      {
          url = "<?php echo site_url('staff/arrear_add')?>";
      }
      else
      {
        url = "<?php echo site_url('staff/arrear_update')?>";
      }
    ...
    {
      if(confirm('Are you sure you want to delete the selected ARREAR DETAILS?'))
      {
        // ajax delete data from database
          $.ajax({
            url : "<?php echo site_url('staff/arrear_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
 ...
 <div id="arrearz" class="modal fade" tabindex="-1"  data-backdrop="static" data-keyboard="false"  style="border-radius:2px;">
				  <div class="modal-header">
						<h4 class="modal-title"> ADD ARREAR</h4>
				  </div>
				  <div class="modal-body">
				<form role="form" action="#" id="formarrear">
				  <div class="col-md-12">
