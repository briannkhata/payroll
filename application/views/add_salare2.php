				...
				<div class="col-md-12">
							...
							<div class="portlet-title">
								...
								<div class="caption">
									...
									<?php echo $page_title;?> | <?php echo $month;?> | <?php echo $year;?></span>
                  ...
                  <?php if ($this->session->flashdata('message')) { ?>
                    <div class="alert alert-success fade in" style="margin-top:4%;">
                      <a href="#" class="close" data-dismiss="alert">&times;</a><center><?php echo $this->session->flashdata('message'); ?> </center>
                    </div>
                  <?php } ?>  
					<form role="form" action="<?php echo base_url();?>salary/create_salare" method="post">
                        <div class="col-md-12">
                          <div class="form-group">
                            <label for="exampleInputEmail1">Staff</label>
                              <select name="employee_id" class="form-control" required="">
                                  <?php
                                  $data = [];
                                  $db = $this->db->select('employee_id')->from('salaries')
                                  ->where('month',$month)
                                  ->where('year',$year)
                                  ->get()->result_array();
                                  if(count($db) > 0)
                                  ...
                                  foreach($ra as $yo):?>
                                <option value="<?php echo $yo['employee_id'];?>"><?php echo $yo['name'];?></option>
                              <?php endforeach;?>
                            </select>
                          </div>
                        </div>
                           ...
                           <a href="<?php echo base_url();?>admin/add_salare1"  class="btn btn-default"><< Back</a>
                          <button type="submit"  class="btn blue zanda"> <i class="fa fa-plus"></i>ADD SALARY</button>
                        </div>
                      </form>
												</div>
										</div>
