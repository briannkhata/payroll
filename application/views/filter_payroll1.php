...
<?php include 'includes/left_nav.php';?>
	<div class="page-content-wrapper">
		<div class="page-content">
			...
			<div class="row">
				<div class="col-md-12">
						<div class="portlet light" style="width: 2000px;">
						...
						<div class="portlet-body">
													...
													<div class="table-toolbar">
															...
															<div class="col-md-6">
																		<div class="btn-group">
																			<form class="form-inline" action="<?=base_url();?>salary/filter_payroll" method="post">
											
																				<select class="form-control" name="month" id="ss2">
																				...
																				<select class="form-control" name="year" id="ss3">
																				...
																				<select class="form-control" name="depot_id" id="ss3">
																					<option selected value="">Filter by Depot</option>
																					<?php $dd=$this->db->get('depot')->result_array();?>
																					<?php foreach($dd as $cel):?>
																						<option value="<?=$cel['depot_id'];?>"><?=$cel['depot'];?></option>
																					<?php endforeach;?>
																				</select>

																				<select class="form-control" name="bank_id" id="ds3">
																					<option selected value="">Filter by Bank</option>
																					<?php $bb=$this->db->get('bank')->result_array();?>
																					<?php foreach($bb as $cel):?>
																						<option value="<?=$cel['bank_id'];?>"><?=$cel['bank'];?></option>
																					<?php endforeach;?>
																				</select>
									...
									<table class="table table-bordered table-striped table-condensed flip-content">
											<thead class="well">
											   <tr>
												...
												<th>PENSION <br> 10%</th>
												<th>PAYE</th>
												<th>NORMAL <br>HOURS</th>
												<th>PUBLIC <br>HOURS</th>
										...
										foreach($payroll as $row):
											...
											$lona  =   $this->db->query("SELECT * FROM bonus WHERE employee_id='$employee_id'");
										    $row1 = $lona->row();
											   if (isset($row1)):
												$bonus	    =	$row1->amount;
											  else:
											...
											$lonaa  =   $this->db->query("SELECT * FROM scheme_bill WHERE employee_id='$employee_id'");
										    $row2 = $lonaa->row();
											   if (isset($row2)):
												$h_bill	    =	$row2->amount;
											  else:
											...
											$sum6 += $row['pension_personal'];
											$sum7 += $row['payee'];
											$sum8 += $row['hourly_rate'];
											$sum9 += $row['broker'];
											$sum10 += $row['gla'];
											$sum11 += $row['normal_hours'];
											$sum12 += $row['public_hours'];
		...
		<tbody>
			  <tr>
				...
				<!--td><?=number_format(sprintf('%0.2f', preg_replace("/[^0-9.]/", "",$row['hourly_rate'])),2);?></td-->
				<td><?=number_format(sprintf('%0.2f', preg_replace("/[^0-9.]/", "",$row['pension_personal'])),2);?></td>
				...
				<td><?=round($row['public_hours'],2);?></td>
