							...
							<div class="tab-pane" id="tab_lon">	
												<div class="portlet-body">
													<div class="table-toolbar">
															...
															<div class="col-md-6">
																		<div class="btn-group">
																			<button id="sample_editable_1_new" class="btn green" onclick="add_department();">
														...
														<?php  
														$count=1;
														$ar =  $this->db->select('*')->from('department')->get()->result_array();
                									    foreach($ar as $row):?>
															<tr class="odd gradeX">
																<td><?=$count++;?></td>
																<td><?=$row['department'];?></td>
																<td><?=$row['description'];?></td>
															    <td><?php if(!empty($this->db->get_where('depot',array('depot_id'=>$row['depot_id']))->row()->depot)): echo $this->db->get_where('depot',array('depot_id'=>$row['depot_id']))->row()->depot; else: echo '';endif;?></td>
																<td align="center">					
																<a href="javascript:;" class="btn btn-success btn-xs black" onclick="edit_department(<?=$row['department_id'];?>)" title="CLICK TO EDIT"><i class="fa fa-edit"></i> EDIT</a>				
															    <a href="javascript:;" class="btn btn-danger btn-xs black" onclick="delete_department(<?=$row['department_id'];?>)" title="CLICK TO DELETE"><i class="fa fa-trash-o"></i> DELETE</a>				
															</td>
															</tr>
															<?php endforeach;?>
															</tbody>
...
<script type="text/javascript">
   var save_method; //for save method string
    ...
    {
      save_method = 'add';
      $('#formdepartment')[0].reset(); // reset form on modals
      $('#department').modal('show'); // show bootstrap modal
    }
    ...
    function edit_department(id)
    {
      ...
      $.ajax({
        url : "<?php echo site_url('staff/department_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('#department').modal('show'); // show bootstrap modal when complete loaded
     ...
     function save()
    {
      ...
      if(save_method == 'add')
      {
          url = "<?php echo site_url('staff/department_add')?>";
      }
      else
      {
        url = "<?php echo site_url('staff/department_update')?>";
      }
	...
	function delete_department(id)
    {
      if(confirm('Are you sure you want to delete the selected DEPARTMENT?'))
      {
        // ajax delete data from database
          $.ajax({
            url : "<?php echo site_url('staff/department_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
 ...
 <div id="department" class="modal fade" tabindex="-1"  data-backdrop="static" data-keyboard="false"  style="border-radius:2px;">
				  <div class="modal-header">
						<h4 class="modal-title"> ADD DEPARTMENT</h4>
				  </div>
				  <div class="modal-body">
				<form role="form" action="#" id="formdepartment">
									
					<div class="col-md-12">
						<div class="form-group">
						  <label for="exampleInputEmail1">Department Name</label>
						      <input type="hidden" class="form-control" name="department_id" value="">
						 	  <input type="text" class="form-control" name="department" id="exampleInputEmail1" placeholder="Department" required>
						</div>
					</div>
					 ...
					 <div class="col-md-12">
						<div class="form-group">
						  <label for="exampleInputEmail1">Depot</label>
						    <select name="depot_id" class="form-control" id="q1">
						       <?php $ra = $this->db->get('depot')->result_array();?>
							   <?php foreach($ra as $yo):?>
									<option value="<?=$yo['depot_id'];?>"><?=$yo['depot'];?></option>
								<?php endforeach;?>
							</select>
						</div>
					</div>
