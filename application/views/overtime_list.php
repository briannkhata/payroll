															...
															<div class="col-md-6">
																	...
																	<form class="form-inline" action="<?=base_url();?>staff/filter_overtime" method="post" style="width:100%; margin-left:25%;margin-top:-7%;">
																		...
																		<select class="form-control see" name="month" style="min-width:34%;">
																			...
																			<?php $this_mon = date('F');
																			$dd=$this->db->get('month')->result_array();?>
																			<?php foreach($dd as $sel):?>
																				<option value="<?=$sel['month'];?>" <?php if($this_mon == $sel['month']) { echo 'selected';}?>>
																					<?=$sel['month'];?> 
																				</option>
																			<?php endforeach;?>
																		</select>
																		...
																		<select class="form-control see" name="year" style="min-width:30%;">
																			...
																			<?php $this_yr = date('Y');
																			$dep=$this->db->get('year')->result_array();?>
																			<?php foreach($dep as $cel):?>
																				<option value="<?=$cel['year'];?>" <?php if($this_yr == $cel['year']) { echo 'selected';}?>>
																					<?=$cel['year'];?>
																				</option>
																			<?php endforeach;?>
																		</select>
																		<button type="submit" class="btn btn-search"><i class="fa fa-search"></i> View Records</button>
																	</form>
						...
						<div class="form-group">
						  <label for="exampleInputEmail1">Month</label>
						    <select name="month" class="form-control see">
							   <?php 
							   $this_month = date('F');
							   $rza = $this->db->get('month')->result_array();?>
							   <?php foreach($rza as $yo):?>
									<option value="<?=$yo['month'];?>" <?php if($this_month == $yo['month']) { echo 'selected';}?>><?=$yo['month'];?></option>
								<?php endforeach;?>
							</select>
						</div>
					</div>
						...
						<div class="form-group">
						  <label for="exampleInputEmail1">Year</label>
						    <select name="year" class="form-control see">
							   <?php $this_yr = date('Y');
							   $raa = $this->db->get('year')->result_array();?>
							   <?php foreach($raa as $yo):?>
									<option value="<?=$yo['year'];?>" <?php if($this_yr == $yo['year']) { echo 'selected';}?>><?=$yo['year'];?></option>
								<?php endforeach;?>
							</select>
						</div>
					</div>
