															...
															<tr class="odd gradeX">
																	...
																	<td><?=$row['scheme_name'];?></td>
																	 <td align="center" class="nop">					
																		<a href="javascript:;" class="btn btn-info btn-xs black" onclick="edit_scheme(<?=$row['scheme_id'];?>)" title="CLICK TO EDIT"><i class="fa fa-edit"></i></a>				
																		<a href="javascript:;" class="btn btn-warning btn-xs black" onclick="delete_scheme(<?=$row['scheme_id'];?>)" title="CLICK TO DELETE"><i class="fa fa-trash-o"></i></a>				
																	</td>
															</tr>
															<?php endforeach;?>
															</tbody>
...
<script type="text/javascript">
   var save_method; //for save method string
	...
	function edit_scheme(id)
    {
      ...
      $.ajax({
        url : "<?php echo site_url('admin/scheme_type_edit/')?>/" + id,
        type: "GET",
        dataType: "JSON",
	...
	function save()
    {
      ...
      if(save_method == 'add')
      {
          url = "<?php echo site_url('admin/scheme_type_add')?>";
      }
      else
      {
        url = "<?php echo site_url('admin/scheme_type_update')?>";
      }
	...
	function delete_scheme(id)
    {
      if(confirm('Are you sure you want to delete the selected SCHEME?'))
      {
        // ajax delete data from database
          $.ajax({
            url : "<?php echo site_url('admin/scheme_type_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
	...
	<div id="scheme" class="modal fade" tabindex="-1"  data-backdrop="static" data-keyboard="false"  style="border-radius:2px;">
				  <div class="modal-header">
						<h4 class="modal-title"> ADD SCHEME TYPE</h4>
				  </div>
					...
					<div class="col-md-12">
						<div class="form-group">
						  <label for="exampleInputEmail1">Scheme Type</label>
						 	  ...
						 	  <input type="text" class="form-control" name="scheme_name" placeholder="Scheme Name" required>
						</div>
					</div>				
