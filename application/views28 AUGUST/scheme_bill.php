												...
												<div class="portlet-body">
														...
														$bill   =   $this->db->select('*')
															...
															<tr class="odd gradeX">
															 ...
															 <td><?php// echo number_format(sprintf('%0.2f', preg_replace("/[^0-9.]/", "",$row['amount'])),2);?>
																<style>
																		#target<?=$row['scheme_bill_id'];?> {
																			  background:white;
																			  width:300px;
																			  height:80px;
																			  padding:5px;
																			...
																			.Hide<?=$row['scheme_bill_id'];?>
																			{
																			  display:none;
																			}			
																			</style>
																		<button class="Show<?=$row['scheme_bill_id'];?> btn btn-success btn-xs black">
																			MK <?php echo number_format(sprintf('%0.2f', preg_replace("/[^0-9.]/", "",$row['amount'])),2);?>
																		</button>
																			<!--<button class="Hide<?php echo $row['employee_id'];?> btn btn-success btn-xs black">Close</button> -->
																			<div id="target<?=$row['scheme_bill_id'];?>">
																				<form action="<?=base_url();?>admin/schemebill_edit_new/update/<?=$row['scheme_bill_id'];?>" method="post">
																				  <input type="text" name="amount" class="form-control" value="<?=$row['amount'];?>"> <br/>
																				  <button type="submit" class="btn btn-success btn-xs black">Save</button>
																				  <button class="Hide<?=$row['scheme_bill_id'];?> btn btn-warning btn-xs black">Close</button> 
																				</form>
																			</div>
																			
																				<script>

																				$('.Show<?=$row['scheme_bill_id'];?>').click(function() {
																					$('#target<?=$row['scheme_bill_id'];?>').show(500);
																					$('.Show<?=$row['scheme_bill_id'];?>').hide(0);
																					$('.Hide<?=$row['scheme_bill_id'];?>').show(0);
																				});
																				$('.Hide<?=$row['scheme_bill_id'];?>').click(function() {
																					$('#target<?=$row['scheme_bill_id'];?>').hide(500);
																					$('.Show<?=$row['scheme_bill_id'];?>').show(0);
																					$('.Hide<?=$row['scheme_bill_id'];?>').hide(0);
																				});
																				$('.toggle').click(function() {
																					$('#target<?=$row['scheme_bill_id'];?>').toggle('slow');
																				});
																				</script>
															 </td>
															 <td align="center" class="nop">	
																...
																<a href="javascript:;" class="btn btn-info btn-xs black" onclick="edit_schemebillQQQQQQQQQQQQQQ(<?=$row['scheme_bill_id'];?>)" title="CLICK TO EDIT"><i class="fa fa-edit"></i> edit</a>				
																<a href="javascript:;" class="btn btn-warning btn-xs black" onclick="delete_schemebill(<?=$row['scheme_bill_id'];?>)" title="CLICK TO DELETE"><i class="fa fa-trash-o"></i> delete</a>				
															</td>
															</tr>
															<?php endforeach;?>
															</tbody>
						...
						<div class="form-group">
						  <label for="exampleInputEmail1">Staff</label>
						   <input type="hidden" name="scheme_bill_id" value="">
						    <select name="employee_id" class="form-control see" required>
