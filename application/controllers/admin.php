...
class Admin extends CI_Controller {
		...
		function salary_changes()
		{
			
			if ($this->session->userdata('account_type') != 'admin')
			redirect(base_url());
			$data['system'] = 'payrollSoft';
			$data['page_title'] = 'Salary Changes';
			$this->load->view('salary_changes',$data);		
				
		}

		function new_salary_change1()
		{
			
			if ($this->session->userdata('account_type') != 'admin')
			redirect(base_url());
			$data['system'] = 'payrollSoft';
			$data['page_title'] = 'Month and Year';
			$this->load->view('new_salary_change1',$data);		
				
		}

		function new_salary_change2()
		{
		...
		function add_salare1(){
			
			if ($this->session->userdata('account_type') != 'admin')
			redirect(base_url());
			$data['system'] = 'payrollSoft';
			$data['page_title'] = 'Add Salary | ';
            $this->load->view('add_salare1',$data);		
				
		}
		function add_salare2(){
			...
			$data['month'] = $this->input->post('month');
			$data['year'] = $this->input->post('year');
            $this->load->view('add_salare2',$data);		
      ...
      'smtp_port' => '465',
      'smtp_user' => 'briannkhata@gmail.com', 
      'smtp_pass' => 'brian12345', 
      'mailtype' => 'html',
      'charset' => 'iso-8859-1',
      'wordwrap' => TRUE
    );
         ...
         $this->email->set_newline("\r\n");
          $this->email->from('briannkhata@gmail.com');
          $this->email->to('nkhata_b@yahoo.com');
		...
		function payslip_pdf($param='',$param2='',$param3='',$param4=''){
			...
			$data['slip'] = $this->db->select('*')
			 ...
			 ->join('bank','bank.bank_id=bank_details.bank_id')
			 ->where('salary_id',$param)
			 ->where('salary_change.employee_id',$param2)
			 ->where('salaries.month',$param3)
			 ->where('salaries.year',$param4)
			 ->where('salary_change.month',$param3)
			 ->where('salary_change.year',$param4)
			 ->get()->result_array();
		   	$this->load->view('payslip1',$data);
...
//substr($myStr, 0, 5);
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A2','IDENTIFY');
		$objPHPExcel->getActiveSheet()->setCellValue('A3','1');
		$objPHPExcel->getActiveSheet()->setCellValue('A4','IDENTIFY');
		$objPHPExcel->getActiveSheet()->setCellValue('A5','11');

		$objPHPExcel->getActiveSheet()->setCellValue('B1','SALARIES');
		$objPHPExcel->getActiveSheet()->setCellValue('B2','VALUE DATE');
		$objPHPExcel->getActiveSheet()->setCellValue('B3',date('Ymd'));
		$objPHPExcel->getActiveSheet()->setCellValue('B4','TOTAL.AMOUNT');

		$objPHPExcel->getActiveSheet()->setCellValue('C2','REFERENCE');
		$objPHPExcel->getActiveSheet()->setCellValue('C3',strtoupper($month.' '.$year).' SAL');
        
        $objPHPExcel->getActiveSheet()->setCellValue('D2','DBT.ACCT');
		$objPHPExcel->getActiveSheet()->setCellValue('D3','955728');


		$objPHPExcel->getActiveSheet()->setCellValue('A6','IDENTIFY');
		$objPHPExcel->getActiveSheet()->setCellValue('B6','AMOUNT');
		$objPHPExcel->getActiveSheet()->setCellValue('C6','BEN.NAME');
		$objPHPExcel->getActiveSheet()->setCellValue('D6','BEN.ACCT');
		$objPHPExcel->getActiveSheet()->setCellValue('E6','BEN.REFERENCE');
		$objPHPExcel->getActiveSheet()->setCellValue('F6','BEN.SORT CODE');
		$objPHPExcel->getActiveSheet()->setCellValue('G6','BEN.BANK');
         
		$ref = strtoupper('SMS  ' .substr($month,0,3).' '.$year).' SAL';
		$row = 7;
		foreach ($bank as $value){
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$row,'10');
			$objPHPExcel->getActiveSheet()->setCellValue('B'.$row,number_format(sprintf('%0.2f', preg_replace("/[^0-9.]/", "",$value['salary'])),2));
			$objPHPExcel->getActiveSheet()->setCellValue('C'.$row,$value['name']);
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$row,$value['account_number']);
			$objPHPExcel->getActiveSheet()->setCellValue('E'.$row,$ref);
			$objPHPExcel->getActiveSheet()->setCellValue('F'.$row,$value['bank_code']);
			$objPHPExcel->getActiveSheet()->setCellValue('G'.$row,strtoupper($value['abbrev']));
			$row++;
		}
		$date = $month.'_'.$year;
		$filename = 'salary_bank_'.$date.'.xls';
		$filename1 = 'salary_bank_'.$date;
		...
		function other_members_add(){
			...
			$membership_id = $this->db->get_where('health_scheme',array('health_scheme_id'=>$data['health_scheme_id']))->row()->membership_id;
			$charge = $this->db->get_where('membership',array('membership_id'=>$membership_id))->row()->charge;

			$toto_bill = $this->db->get_where('health_scheme',array('health_scheme_id'=>$data['health_scheme_id']))->row()->total_bill;
			if($toto_bill != 0){

				//$total_bill = $toto_bill  + $charge;
				$fuck['total_bill'] = $data['d_amount'] + $toto_bill;
				//$fuck['total_bill'] = $data['d_amount'] + $charge;

			}
			else{
				$total_bill = $toto_bill  + $charge;
				$fuck['total_bill'] = $data['d_amount'] + $total_bill;
			}
			//$this->db->where('health_scheme_id',$data['health_scheme_id']);
			//$this->db->update('health_scheme',$qq);			
	...
	####BOOIKNH FUNCTIONS####
	    ...
	    function add_health()
		{
            ...
            redirect(base_url(), 'refresh');
			$data['count']=1;
			$data['page_title']  = 'New Health';
			$data['system']  = 'PayrollSoft';
			$this->load->view('add_health',$data);			
		}
		...
		function scheme_add()
		{
			...
			$health_scheme_id = $this->db->insert_id();


			$zoba['health_scheme_id'] = $health_scheme_id;
			$other_members = $this->input->post('other_members');
			$deduct = $this->input->post('deduct');
			$d_amount = $this->input->post('d_amount');	
			
			$membership_id = $this->db->get_where('health_scheme',array('health_scheme_id'=>$health_scheme_id))->row()->membership_id;
			$charge = $this->db->get_where('membership',array('membership_id'=>$membership_id))->row()->charge;
			$toto_bill = $this->db->get_where('health_scheme',array('health_scheme_id'=>$health_scheme_id))->row()->total_bill;
			$total_bill = $toto_bill  + $charge;

			foreach ($other_members as $key => $value){
				if($toto_bill != 0){
				//$total_bill = $toto_bill  + $charge;
				$fuck['total_bill'] = $d_amount[$key] + $toto_bill;
				//$fuck['total_bill'] = $data['d_amount'] + $charge;
				}else{
				$fuck['total_bill'] = $d_amount[$key] + $total_bill;
			   }

				$zoba['other_members'] = $value;
				$zoba['deduct'] = $deduct[$key];
				$zoba['d_amount'] = $d_amount[$key];	

			 	$this->db->insert('other_members',$zoba);
				$fuck['other_members'] = 1 ;
